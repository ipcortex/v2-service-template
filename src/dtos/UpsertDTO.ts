...
export class UpsertDTO {
  ...
  @Matches(stringRegEx, {message: 'label can contain only aplha numeric characters and .-_ '})
    label!: string;
  ...
  @MaxDate(new Date(Date.now()-1000), { message: 'lastRegistered cannot be in the future' })
    lastRegistered?: Date;
  ...
  @IsDate({ message: 'lastRegistered must be a valid date' })
  @MinDate(new Date(Date.now()+3600000), { message: 'registrationExpiry has to be at least 1 hour in the future' })
  ...
  @Matches(stringRegEx, {message: 'username can contain only aplha numeric characters and .-_ '})
    username?: string;
  ...
  @Matches(passwordRegEx, {message: 'password can contain only aplha numeric characters and .-_#$ '})
    password?: string;
